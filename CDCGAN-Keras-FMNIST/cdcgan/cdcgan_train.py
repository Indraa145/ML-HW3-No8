...
for epoch in range(EPOCHS):
    ...
    for i in range(nb_of_iterations_per_epoch):
        ...
        if i % 20 == 0:
            image_grid = cdcgan_utils.generate_fashion_mnist_image_grid(G,
                                                                title="Epoch {0}, iteration {1}".format(epoch,
                                                                                                        iteration))
            cdcgan_utils.save_generated_image(image_grid, epoch, i, "../images/generated_fashion_mnist_images_per_iteration")
            image_logger.log_images("generated_fashion_mnist_images_per_iteration", [image_grid], iteration)
    ...
    image_grid = cdcgan_utils.generate_fashion_mnist_image_grid(G, title="Epoch {0}".format(epoch))
    cdcgan_utils.save_generated_image(image_grid, epoch, 0, "../images/generated_fashion_mnist_images_per_epoch")
    image_logger.log_images("generated_fashion_mnist_images_per_epoch", [image_grid], epoch)

    pbar.close()
    print("D loss: {0}, G loss: {1}".format(np.mean(d_losses_for_epoch), np.mean(g_losses_for_epoch)))

    G.save_weights("../models/weights/generator_fashion_mnist.h5")
    D.save_weights("../models/weights/discriminator_fashion_mnist.h5")
